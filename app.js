...
window.copyBriefPrompt=(id,field)=>{const t=task(id);if(t)copyText(briefFor(t)[field]||"")}
window.openTaskBrief=id=>{const t=task(id);if(!t)return;const b=briefFor(t),resources=b.resources||[],required=resources.filter(r=>r.required),optional=resources.filter(r=>!r.required),requiredVideos=required.filter(r=>r.type==="VIDEO"),requiredMaterials=required.filter(r=>r.type!=="VIDEO");const resourceList=list=>list.map(r=>`<div class="task"><strong>${esc(r.title)}</strong><div class="task-meta"><span class="badge">${esc(r.type||"RESOURCE")}</span><span class="badge">${esc(r.source||"To verify")}</span><span class="badge">${r.required?"Required":"Optional"}</span></div><p class="muted">${esc(r.instruction||"")}</p>${r.url?`<a class="btn small" href="${esc(r.url)}" target="_blank" rel="noopener">Open Resource</a>`:'<span class="muted">To verify</span>'}</div>`).join("")||"<p class='muted'>None.</p>";const resourceSection=(title,list)=>`<h3>${title}</h3>${resourceList(list)}`;openModal(t.title,`<div class="task-meta"><span class="badge">${esc(b.taskType)}</span><span class="badge">${b.estimatedMinutes||t.estimatedMinutes} min</span></div><h3>Objective</h3><p>${esc(b.objective)}</p><h3>Why this matters</h3><p>${esc(b.whyItMatters)}</p><h3>Exact steps</h3><ol>${(b.steps||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ol>${resourceSection("Required documents / resources",requiredMaterials)}${resourceSection("Required videos",requiredVideos)}${resourceSection("Optional resources",optional)}<h3>NotebookLM prompt</h3><div class="prompt">${esc(b.notebookLMPrompt||"")}</div><button class="btn small" onclick="copyBriefPrompt('${t.id}','notebookLMPrompt')">Copy NotebookLM prompt</button><h3>ChatGPT prompt</h3><div class="prompt">${esc(b.chatGPTPrompt||"")}</div><button class="btn small" onclick="copyBriefPrompt('${t.id}','chatGPTPrompt')">Copy ChatGPT prompt</button><h3>Deliverable</h3><p>${esc(b.deliverable||"")}</p><h3>Done when</h3><ul>${(b.completionCriteria||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ul><h3>Evidence / notes</h3><label>Note<textarea id="briefNotes">${esc(t.notes||"")}</textarea></label><label>Output link / reference<input id="briefLink" class="input" value="${esc(t.evidenceLink||"")}"></label><label>Completion evidence<textarea id="briefEvidence">${esc(t.evidence||"")}</textarea></label><p class="muted">${esc(b.evidencePrompt||"")}</p><div class="task-actions"><button class="btn primary" onclick="saveBrief('${t.id}')">Save evidence</button><button class="btn" onclick="finish('${t.id}')">Mark complete</button></div><h3>Prerequisites</h3><p>${(b.prerequisites||[]).map(x=>esc(task(x)?.title||x)).join(", ")||"None."}</p><h3>Next task</h3><p>${b.nextTaskId?esc(task(b.nextTaskId)?.title||b.nextTaskId):"Follow the next unfinished scheduled task."}</p><p class="muted">${esc(b.statusNote||"")}</p>`)}
